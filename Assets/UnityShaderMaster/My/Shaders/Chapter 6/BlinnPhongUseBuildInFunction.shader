Shader "Unity_Shaders_Book/Chapter 6/BlinnPhongUseBuildInFunction"
{
    Properties {
    ...
    SubShader{
        Pass {
            ...
            v2f vert(a2v v) {
                ...
                o.worldNormal = UnityObjectToWorldNormal(v.normal);   //  向片元着色器传递世界空间的法线方向
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                fixed3 worldLightDir = normalize(UnityWorldSpaceLightDir(i.worldPos));     //  得到世界空间的光照方向（平行光）

                fixed3 diffuse = _LightColor0.rgb * _Diffuse.rgb * saturate(dot(worldNormal, worldLightDir));   //  计算漫反射

                fixed3 viewDir = normalize(UnityWorldSpaceViewDir(i.worldPos));  //  计算观察方向
